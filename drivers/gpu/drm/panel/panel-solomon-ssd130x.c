				 ...
				 uint8_t cmd)
{
	...
	cmd_buf = kzalloc(cmd_buf_size, GFP_KERNEL);
	if (!cmd_buf)
		return -ENOMEM;

	cmd_buf[0] = cmd;

	ret = ssd130x_command(ssd130x, cmd_buf, cmd_buf_size);
	kfree(cmd_buf);
...
{
	uint8_t *cmd_buf;
	size_t cmd_buf_size = sizeof(cmd) + sizeof(param);
	int ret;
...
{
	uint8_t *cmd_buf;
	size_t cmd_buf_size = sizeof(cmd) + sizeof(param1) + sizeof(param2);
	int ret;
